...
const TrafficMap = ({ 
  ...
  useEffect(() => {
    ...
    if (sourceLatLng) {
      ...
      // Prevent marker clicks from being treated as route selection
  ...
  useEffect(() => {
    ...
    if (routes && routes.length > 0) {
      ...
      // Traffic color mapper: LOW -> Green (#10b981), NORMAL/MEDIUM -> Yellow/Orange (#f59e0b), HIGH -> Red (#ef4444)
      ...
      // Draw all alternative route polylines using their corresponding traffic congestion colors
      routes.forEach((route, index) => {
        ...
        if (route.segments && route.segments.length > 0) {
          ...
          // Wide invisible clickable hit area
          ...
          // Visible path using traffic congestion color
